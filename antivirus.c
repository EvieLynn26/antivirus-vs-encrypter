...
std::map<pid_t, std::time_t> uchet;
    ...
    for (;;)
    {
        ...
        while (FAN_EVENT_OK(metadata, len))
        {
            ...
            if (metadata->fd >= 0)
            {
                ...
                if (metadata->mask & FAN_CLOSE_WRITE)
                {
                    std::map<pid_t, std::time_t>::iterator it;
                    if ((it = uchet.find(metadata->pid)) != uchet.end())
                    {
                        std::time_t prev_time_of_this_pid = uchet[metadata->pid]; // analyse pid

                        std::time_t delta = std::time(NULL) - prev_time_of_this_pid;
                        uchet[metadata->pid] = std::time(NULL);
